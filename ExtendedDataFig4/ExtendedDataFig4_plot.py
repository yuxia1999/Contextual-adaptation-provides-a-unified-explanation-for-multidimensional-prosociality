...
def plot_pq_vs_u_withSimulation(file_pattern, deg_values, M_numbers, gamma, selection_intensity=0.001):
    ...
    colors = ['#c1272c', '#4758A2', '#0070bb', '#0070bb']  
    ...
    for i, M_number in enumerate(M_numbers):
        ...
        for deg in deg_values:
            ...
            plt.plot(u_values_filtered, q_values, linewidth=2.0, label=rf'M={M_number}, $\langle q \rangle$', color=colors[i], linestyle='--')
    ...
    for index in range(1,ite+1):
        filename30 = f'EDF4_data/data_figure5_panel_a_M30_gamma1.0_selectionIntenstiy0.005_100000000times_{index}.txt'
        filename50 = f'EDF4_data/data_figure5_panel_a_M50_gamma1.0_selectionIntenstiy0.005_100000000times_{index}.txt'
    ...
    # # plt.savefig(rf"ExtendedDataFig4_ultimatumGame_pqFrequency_withSimulation_SF_degree{deg_values[0]}_new.svg")
    plt.show()
...
def plot_pq_vs_gamma_variousMutationRate(file_pattern, deg_values, M_numbers, mutation_rates,selection_intensity=0.005):
    ...
    for i, mutation_rate in enumerate(mutation_rates):
        for deg in deg_values:
            ...
            omega_values = []
            for l1, l2, l3 in zip(lambda1_filtered, lambda2_filtered, lambda3_filtered):
                phi,omega =  ultimatumGame_pq_frequency_continual(l1, l2, l3, selection_intensity)
            ...
            plt.plot(gamma_values_filtered, omega_values, linewidth=2.0, label=rf'$\mu$={mutation_rate}, $\langle q \rangle$', color=colors[i], linestyle='--')
    ...
    # plt.savefig(rf"ExtendedDataFig4_UG_pqFrequency_vs_gamma_verious_u_SF_degree{deg_values}_M{M_number}_delta{selection_intensity}.svg")
    plt.show()
...
def plot_pq_vs_M_SF_RR_variousMutationRate_line(deg, M_value, gamma, selection_intensity=0.005):
    ...
    for index in range(1, ite_number+1):
        file_name = f'EDF4_data/StructureCoefficient_RR_DB_30_degree{deg}_{index}_M{M_value}_u_variation.txt'
        data = np.loadtxt(file_name)
        gamma_values, u_values, lambda1_values, lambda2_values, lambda3_values = data.T
        ...
        file_name = f'EDF4_data/StructureCoefficient_SF_DB_30_degree{deg}_{index}_M{M_value}_u_variation.txt'
        data = np.loadtxt(file_name)
        gamma_values, u_values, lambda1_values, lambda2_values, lambda3_values = data.T
    ...
    plt.plot(u_values_filtered, accumulated_r_RR, label=rf'RR, $\langle q \rangle$', color=colors[1], linewidth=2.0, linestyle='--')
    plt.plot(u_values_filtered, accumulated_p_SF, label=rf'BA, $\langle p \rangle$', color=colors[0], linewidth=2.0)
    plt.plot(u_values_filtered, accumulated_r_SF, label=rf'BA, $\langle q \rangle$', color=colors[0], linewidth=2.0, linestyle='--')
    ...
    # plt.savefig(rf"ExtendedDataFig4_UG_prFrequency_vs_M_SF_RR_M{M_value}_gamma{gamma}_delta{selection_intensity}_line.svg")
    plt.show()
...
def plot_pq_vs_gamma_variousNetwork(file_pattern, deg_values, M_numbers,  mutation_rates,selection_intensity=0.005):
    ...
    for i, mutation_rate in enumerate(mutation_rates):
        for deg in deg_values:
            ...
            plt.plot(gamma_values_filtered, omega_values, linewidth=2.0, label=rf'SF, $\langle q \rangle$', color=colors[i], linestyle='--')

    file_name = "EDF4_data/SC_RR_DB_30_degree4_1_M_variation.txt"
    data = np.loadtxt(file_name)
    ...
    plt.plot(gamma_values_filtered, omega_values, linewidth=2.0, label=rf'RR, $\langle q \rangle$', color=colors[1], linestyle='--')
    ...
    # plt.savefig(rf"ExtendedDataFig4_UG_gamma_heterogeneity_degree{deg_values}_M{M_number}_delta{selection_intensity}.svg")
    plt.show()

plot_pq_vs_u_withSimulation("EDF4_data/StructureCoefficient_SF_DB_30_degree{deg}_1_M{M_number}_u_variation.txt", [4], [30, 40, 50, 60], 1.0,0.005) # panel a

plot_pq_vs_gamma_variousMutationRate("EDF4_data/StructureCoefficient_SF_DB_30_degree{deg}_1_M{M_number}_gamma_variation.txt", [4], [50], [0.01,0.1], 0.005) # panel b

plot_pq_vs_M_SF_RR_variousMutationRate_line(4, 50, 1.0, 0.005) # panel c

plot_pq_vs_gamma_variousNetwork("EDF4_data/StructureCoefficient_SF_DB_30_degree{deg}_1_M{M_number}_gamma_variation.txt", [4], [50], [0.01], 0.005) # panel d
