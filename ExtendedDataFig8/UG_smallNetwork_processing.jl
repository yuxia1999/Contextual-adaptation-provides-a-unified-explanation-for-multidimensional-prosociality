...
p_ori,q_ori = ultimatumGame_pq_frequency_continual(lambda1_ori, lambda2_ori, lambda3_ori, selection_intensity)
p_grouped,q_grouped = ultimatumGame_pq_frequency_continual(lambda1_grouped, lambda2_grouped, lambda3_grouped, selection_intensity)
open("ultimatumGame_networkOfSize6_p_ungrouped_u0.1.txt", "a") do io
        println(io, @sprintf("%.10f", p_ori))
end
open("ultimatumGame_networkOfSize6_q_ungrouped_u0.1.txt", "a") do io
    println(io, @sprintf("%.10f", q_ori))
end
open("ultimatumGame_networkOfSize6_p_grouped_u0.1.txt", "a") do io
    println(io, @sprintf("%.10f", p_grouped))
end
open("ultimatumGame_networkOfSize6_q_grouped_u0.1.txt", "a") do io
