...
function DTG_bcCritical(lambda1, lambda2, lambda3, selection_intensity)
...
bc_ori = DTG_bcCritical(lambda1_ori, lambda2_ori, lambda3_ori, selection_intensity)
bc_grouped = DTG_bcCritical(lambda1_grouped, lambda2_grouped, lambda3_grouped, selection_intensity)
open("DTG_networkOfSize6_bc_ungrouped_u0.1.txt", "a") do io
        println(io, @sprintf("%.10f", bc_ori))
end

open("DTG_networkOfSize6_bc_grouped_u0.1.txt", "a") do io
